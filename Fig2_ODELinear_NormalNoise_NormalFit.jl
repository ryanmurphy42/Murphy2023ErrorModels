...
# Fig2_ODELinear_NormalNoise_NormalFit
...
isdir(pwd() * "\\Fig2_ODELinear_NormalNoise_NormalFit\\") || mkdir(pwd() * "\\Fig2_ODELinear_NormalNoise_NormalFit") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\Fig2_ODELinear_NormalNoise_NormalFit\\"] # location to save figures "\\Fig2\\"] # location to save figures
...
## Generate Synthetic data
    ...
    data_dists=[Normal(mi,Sd) for mi in model(t,[r1,r2,C10,C20])]; # generate noisy data using normal distribution
...
### Parameter identifiability analysis - MLE, profiles
    ...
    data0_smooth_MLE = model(t_smooth,[r1mle,r2mle,C10,C20]); # model solution at MLE for plotting
    ...
    p1=scatter!(t,data[1,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour1) # scatter c1
    p1=scatter!(t,data[2,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour2) # scatter c2
    ...
    #######################################################################################
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr1range_upper[2,i]) for mi in model(t_smooth,[r1range_upper[i],nr1range_upper[1,i]])]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr1range_lower[2,i]) for mi in model(t_smooth,[r1range_lower[i],nr1range_lower[1,i]])]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr2range_upper[2,i]) for mi in model(t_smooth,[nr2range_upper[1,i],r2range_upper[i]])]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr2range_lower[2,i]) for mi in model(t_smooth,[nr2range_lower[1,i],r2range_lower[i]])]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            nSdrange_upper[:,i]=xo[:]
            llSd_upper[i]=fo[1]
            predict_Sd_upper[:,:,i]=model(t_smooth,[nSdrange_upper[1,i],nSdrange_upper[2,i]])


            loop_data_dists=[Normal(mi,Sdrange_upper[i]) for mi in model(t_smooth,[nSdrange_upper[1,i],nSdrange_upper[2,i]])]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            nSdrange_lower[:,i]=xo[:]
            llSd_lower[i]=fo[1]
            predict_Sd_lower[:,:,i]=model(t_smooth,[nSdrange_upper[1,i],nSdrange_upper[2,i]])

            loop_data_dists=[Normal(mi,Sdrange_lower[i]) for mi in model(t_smooth,[nSdrange_lower[1,i],nSdrange_lower[2,i]])]; # normal distribution about mean
        ...
        max_Sd_Sd= zeros(2,length(t_smooth))
        min_Sd_Sd= zeros(2,length(t_smooth))
        for i in 1:(nptss)
        ...
        profile3=plot(interp_points_Sdrange,interp_nllSd,xlim=(Sdmin,Sdmax),ylim=(-4,0.1),yticks=[-3,-2,-1,0],xlab=L"\sigma_{N}",ylab=L"\hat{\ell}_{p}",legend=false,lw=5,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=:deepskyblue3)
        ...
        p_residuals=scatter(t,data_residuals[1,:],xlims=(xmin,xmax),ylims=(-15,15),legend=false,markersize = 15,markercolor=colour1,markerstrokewidth=0,yticks=[-10,0,10]) # scatter c1
        p_residuals=scatter!(t,data_residuals[2,:],xlims=(xmin,xmax),ylims=(-15,15),legend=false,markersize =15,markercolor=colour2,xlab=L"t",ylab=L"\hat{e}_{i}",lw=3,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,markerstrokewidth=0) # scatter c2        
        ...
        # plot qq-plot
        p_residuals_qqplot = plot(qqplot(Normal,[data_residuals[1,:];data_residuals[2,:]],lw=2,linecolor=:black,linestyle=:dot,markersize = 8,markercolor=:black),legend=false,xlab=L"\mathrm{Normal}",ylab=L"\mathrm{Residuals}",lw=3,titlefont=fnt, guidefont=fnt, tickfont=fnt,ylims=(-15,15),yticks=[-10,0,10])
...
#########################################################################################
        ...
        confmodel3=scatter!(t,data[1,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour1,msw=0) # scatter c1
        confmodel3=scatter!(t,data[2,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour2,msw=0) # scatter c2
...
#########################################################################################
        ...
        confmodeldiff3=plot!(t_smooth,0 .*ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_Sd[2,:],max_Sd[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{y,0.95}^{\sigma_{\mathrm{N}}} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
...
#########################################################################################
    ...
    ##############################
        ...
        # r1
        confrealdiff1=plot(t_smooth,0 .* ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_realisations_r1[1,:],max_realisations_r1[1,:].-ymle_smooth[1,:]),ylims=(ynoisediffmin,ynoisediffmax),legend=false,fillalpha=.2,lw=0,linecolor=colour1)
        confrealdiff1=plot!(t_smooth,0 .* ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_realisations_r1[2,:],max_realisations_r1[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{z_{i},0.95}^{r_{1}} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
        confrealdiff1=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash)

        display(confrealdiff1)
        savefig(confrealdiff1,filepath_save[1] * "Fig_confrealdiff1"   * ".pdf")

        # r2
        confrealdiff2=plot(t_smooth,0 .* ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_realisations_r2[1,:],max_realisations_r2[1,:].-ymle_smooth[1,:]),ylims=(ynoisediffmin,ynoisediffmax),legend=false,fillalpha=.2,lw=0,linecolor=colour1)
        confrealdiff2=plot!(t_smooth,0 .* ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_realisations_r2[2,:],max_realisations_r2[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{z_{i},0.95}^{r_{2}} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
        confrealdiff2=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash)

        display(confrealdiff2)
        savefig(confrealdiff2,filepath_save[1] * "Fig_confrealdiff2"   * ".pdf")

        #Sd
        confrealdiff3=plot(t_smooth,0 .* ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_realisations_Sd[1,:],max_realisations_Sd[1,:].-ymle_smooth[1,:]),ylims=(ynoisediffmin,ynoisediffmax),legend=false,fillalpha=.2,lw=0,linecolor=colour1)
        confrealdiff3=plot!(t_smooth,0 .* ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_realisations_Sd[2,:],max_realisations_Sd[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{z_{i},0.95}^{\sigma_{\mathrm{N}}} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
        confrealdiff3=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash)

        display(confrealdiff3)
        savefig(confrealdiff3,filepath_save[1] * "Fig_confrealdiff3"   * ".pdf")


        # overall - plot model simulated at MLE, scatter data, and prediction interval (union of confidence sets for realisations)

        confrealdiffu=plot(t_smooth,0 .* ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_realisations_overall[1,:],max_realisations_overall[1,:].-ymle_smooth[1,:]),ylims=(ynoisediffmin,ynoisediffmax),legend=false,fillalpha=.2,lw=0,linecolor=colour1)
        confrealdiffu=plot!(t_smooth,0 .* ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_realisations_overall[2,:],max_realisations_overall[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{z_{i},0.95} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
        confrealdiffu=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash)
