...
# FigS11_Coverage_ODELinear_LogNormalNoise_LogNormalFit_FullLikelihood
...
isdir(pwd() * "\\FigS11_Coverage_ODELinear_LogNormalNoise_LogNormalFit_FullLikelihood\\") || mkdir(pwd() * "\\FigS11_Coverage_ODELinear_LogNormalNoise_LogNormalFit_FullLikelihood") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\FigS11_Coverage_ODELinear_LogNormalNoise_LogNormalFit_FullLikelihood\\"] # location to save figures "\\Fig2\\"] # location to save figures
        ...
        for i in 1:length(r1rangefull)
            for j in 1:length(r2rangefull)
                if fullloglikelihood[i,j] > THreal
                    ...
                    loop_data_dists=[LogNormal(log(mi),Sd) for mi in model(t,a)]; # normal distribution about mean
