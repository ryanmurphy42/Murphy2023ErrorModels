...
# FigS6_ODElotkavolterra_PoissonNoise_NormalFit2
...
isdir(pwd() * "\\FigS6_ODElotkavolterra_PoissonNoise_NormalFit2\\") || mkdir(pwd() * "\\FigS6_ODElotkavolterra_PoissonNoise_NormalFit2") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\FigS6_ODElotkavolterra_PoissonNoise_NormalFit2\\"] # location to save figures "\\Fig2\\"] # location to save figures
filepath_load = [pwd() * "\\FigS5_ODElotkavolterra_PoissonNoise_PoissonFit\\"]
...
C20=10.0;
t_max = 100.0;
t=LinRange(5,t_max, 21); # synthetic data measurement times
...
#######################################################################################
## Load synthetic data generated using the Poisson noise model

    df_data= CSV.read(filepath_load[1] * "Data.csv",DataFrame)
    data = [transpose(df_data[:,1]); transpose(df_data[:,2])]
...
### Parameter identifiability analysis - MLE, profiles
    ...
    function error(data,a)
        ...
        y=model(t,a);
        e=0;
        data_dists=[Normal(mi,a[5]) for mi in y]; # Normal noise model
    ...
    θG = [VV1,KK1,VV2,KK2,Sd] # first guess
    ...
    lb=[V1_lb,K1_lb,V2_lb,K2_lb,Sd_lb];
    ub=[V1_ub,K1_ub,V2_ub,K2_ub,Sd_ub];
    ...
    savefig(p1,filepath_save[1] * "Fig1" * ".pdf")
        ...
        for i in 1:nptss
            function fun1(aa)
                return error(data,[P1range_upper[i],aa[1],aa[2],aa[3],aa[4]])
            end
            lb1=[lb[2],lb[3],lb[4],lb[5]];
            ub1=[ub[2],ub[3],ub[4],ub[5]];
            local θG1=[P2mle, P3mle, P4mle,P5mle]
            ...
            loop_data_dists=[Normal(mi,P1nrange_upper[4,i]) for mi in modelmean];
            ...
            if fo > fmle
                ...
                global P5mle=P1nrange_upper[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun1a(aa)
                return error(data,[P1range_lower[i],aa[1],aa[2],aa[3],aa[4]])
            end
            lb1=[lb[2],lb[3],lb[4],lb[5]];
            ub1=[ub[2],ub[3],ub[4],ub[5]];
            local θG1=[P2mle, P3mle, P4mle,P5mle]
            ...
            modelmean = model(t_smooth,[P1range_lower[i],P1nrange_lower[1,i],P1nrange_lower[2,i],P1nrange_lower[3,i],P1nrange_lower[4,i]])
            predict_P1_lower[:,:,i]=modelmean

            loop_data_dists=[Normal(mi,P1nrange_lower[4,i]) for mi in modelmean]; # normal distribution about mean
            ...
            if fo > fmle
                ...
                global P5mle=P1nrange_lower[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun2(aa)
                return error(data,[aa[1],P2range_upper[i],aa[2],aa[3],aa[4]])
            end
            lb1=[lb[1],lb[3],lb[4],lb[5]];
            ub1=[ub[1],ub[3],ub[4],ub[5]];
            local θG1=[P1mle, P3mle, P4mle,P5mle]
            local (xo,fo)=optimise(fun2,θG1,lb1,ub1)
            P2nrange_upper[:,i]=xo[:]
            llP2_upper[i]=fo[1]

            modelmean = model(t_smooth,[P2nrange_upper[1,i],P2range_upper[i],P2nrange_upper[2,i],P2nrange_upper[3,i],P2nrange_upper[4,i]])
            predict_P2_upper[:,:,i]= modelmean

            loop_data_dists=[Normal(mi,P2nrange_upper[4,i]) for mi in modelmean]; # normal distribution about mean
            ...
            if fo > fmle
                ...
                global P5mle=P2nrange_upper[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun2a(aa)
                return error(data,[aa[1],P2range_lower[i],aa[2],aa[3],aa[4]])
            end
            lb1=[lb[1],lb[3],lb[4],lb[5]];
            ub1=[ub[1],ub[3],ub[4],ub[5]];
            local θG1=[P1mle, P3mle, P4mle,P5mle]
            ...
            loop_data_dists=[Normal(mi,P2nrange_lower[4,i]) for mi in modelmean]; # normal distribution about mean
            ...
            if fo > fmle
                ...
                global P5mle=P2nrange_lower[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun3(aa)
                return error(data,[aa[1],aa[2],P3range_upper[i],aa[3],aa[4]])
            end
            lb1=[lb[1],lb[2],lb[4],lb[5]];
            ub1=[ub[1],ub[2],ub[4],ub[5]];
            local θG1=[P1mle,P2mle,P4mle,P5mle]    
            ...
            loop_data_dists=[Normal(mi,P3nrange_upper[4,i]) for mi in modelmean]; # normal distribution about mean
            ...
            if fo > fmle
                ...
                global P5mle=P3nrange_upper[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun3a(aa)
                return error(data,[aa[1],aa[2],P3range_lower[i],aa[3],aa[4]])
            end
            lb1=[lb[1],lb[2],lb[4],lb[5]];
            ub1=[ub[1],ub[2],ub[4],ub[5]];
            local θG1=[P1mle,P2mle,P4mle,P5mle]      
            ...
            loop_data_dists=[Normal(mi,P3nrange_lower[4,i]) for mi in modelmean]; # normal distribution about mean
            ...
            if fo > fmle
                ...
                global P5mle=P3nrange_lower[4,i]
            end
        end
        ...
        for i in 1:(nptss)
         ...
         for i in 1:nptss
             function fun4(aa)
                 return error(data,[aa[1],aa[2],aa[3],P4range_upper[i],aa[4]])
             end
             lb1=[lb[1],lb[2],lb[3],lb[5]];
             ub1=[ub[1],ub[2],ub[3],ub[5]];
             local θG1=[P1mle,P2mle,P3mle,P5mle]
             ...
             loop_data_dists=[Normal(mi,P4nrange_upper[4,i]) for mi in modelmean]; # normal distribution about mean
             ...
             if fo > fmle
                 ...
                 global P5mle=P4nrange_upper[4,i]
             end
         end
         ...
         for i in 1:nptss
             function fun4a(aa)
                 return error(data,[aa[1],aa[2],aa[3],P4range_lower[i],aa[4]])
             end
             lb1=[lb[1],lb[2],lb[3],lb[5]];
             ub1=[ub[1],ub[2],ub[3],ub[5]];
             local θG1=[P1mle,P2mle,P3mle,P5mle]
             ...
             loop_data_dists=[Normal(mi,P4nrange_lower[4,i]) for mi in modelmean]; # normal distribution about mean
             ...
             if fo > fmle
                 ...
                 global P5mle=P4nrange_lower[4,i]
             end
         end
        ...
        P5min=lb[5]
        P5max=ub[5]
        P5range_lower=reverse(LinRange(P5min,P5mle,nptss))
        P5range_upper=LinRange(P5mle + (P5max-P5mle)/nptss,P5max,nptss)
        
        
        P5nrange_lower=zeros(4,nptss)
        llP5_lower=zeros(nptss)
        nllP5_lower=zeros(nptss)
        predict_P5_lower=zeros(2,length(t_smooth),nptss)
        predict_P5_realisations_lower_lq=zeros(2,length(t_smooth),nptss)
        predict_P5_realisations_lower_uq=zeros(2,length(t_smooth),nptss)

        P5nrange_upper=zeros(4,nptss)
        llP5_upper=zeros(nptss)
        nllP5_upper=zeros(nptss)
        predict_P5_upper=zeros(2,length(t_smooth),nptss)
        predict_P5_realisations_upper_lq=zeros(2,length(t_smooth),nptss)
        predict_P5_realisations_upper_uq=zeros(2,length(t_smooth),nptss)
        
        # start at mle and increase parameter (upper)
        for i in 1:nptss
            function fun5(aa)
                return error(data,[aa[1],aa[2],aa[3],aa[4],P5range_upper[i]])
            end
            lb1=[lb[1],lb[2],lb[3],lb[4]];
            ub1=[ub[1],ub[2],ub[3],ub[4]];
            local θG1=[P1mle,P2mle,P3mle,P4mle]
            local (xo,fo)=optimise(fun5,θG1,lb1,ub1)
            P5nrange_upper[:,i]=xo[:]
            llP5_upper[i]=fo[1]

            modelmean = model(t_smooth,[P5nrange_upper[1,i],P5nrange_upper[2,i],P5nrange_upper[3,i],P5nrange_upper[4,i]])
            predict_P5_upper[:,:,i]=modelmean

            loop_data_dists=[Normal(mi,P5range_upper[i]) for mi in modelmean]; # normal distribution about mean
            predict_P5_realisations_upper_lq[:,:,i]= [quantile(data_dist, THalpha/2) for data_dist in loop_data_dists];
            predict_P5_realisations_upper_uq[:,:,i]= [quantile(data_dist, 1-THalpha/2) for data_dist in loop_data_dists];


            if fo > fmle
                println("new MLE P5 upper")
                global fmle = fo
                global P1mle=P5nrange_upper[1,i]
                global P2mle=P5nrange_upper[2,i]
                global P3mle=P5nrange_upper[3,i]
                global P4mle=P5range_upper[i]
                global P5mle=P5nrange_upper[4,i]
            end
        end
        ...
        for i in 1:nptss
            function fun5a(aa)
                return error(data,[aa[1],aa[2],aa[3],aa[4],P5range_lower[i]])
            end
            lb1=[lb[1],lb[2],lb[3],lb[4]];
            ub1=[ub[1],ub[2],ub[3],ub[4]];
            local θG1=[P1mle,P2mle,P3mle,P4mle]
            ...
            modelmean =  model(t_smooth,[P5nrange_lower[1,i],P5nrange_lower[2,i],P5nrange_lower[3,i],P5nrange_lower[4,i]])
            predict_P5_lower[:,:,i]= modelmean

            loop_data_dists=[Normal(mi,P5range_lower[i]) for mi in modelmean]; # normal distribution about mean
            predict_P5_realisations_lower_lq[:,:,i]= [quantile(data_dist, THalpha/2) for data_dist in loop_data_dists];
            predict_P5_realisations_lower_uq[:,:,i]= [quantile(data_dist, 1-THalpha/2) for data_dist in loop_data_dists];


            if fo > fmle
                println("new MLE P5 lower")
                global fmle = fo
                global P1mle=P5nrange_lower[1,i]
                global P2mle=P5nrange_lower[2,i]
                global P3mle=P5nrange_lower[3,i]
                global P4mle=P5nrange_lower[4,i]
                global P5mle=P5range_lower[i]
            end
        end
        
        # combine the lower and upper
        P5range = [reverse(P5range_lower);P5range_upper]
        llP5 = [reverse(llP5_lower); llP5_upper]        
        nllP5=llP5.-maximum(llP5);
        ...
        max_P5=zeros(2,length(t_smooth))
        min_P5=100000*ones(2,length(t_smooth))
        for i in 1:(nptss)
            if (llP5_lower[i].-maximum(llP5)) >= TH
                for j in 1:length(t_smooth)
                    max_P5[1,j]=max(predict_P5_lower[1,j,i],max_P5[1,j])
                    min_P5[1,j]=min(predict_P5_lower[1,j,i],min_P5[1,j])
                    max_P5[2,j]=max(predict_P5_lower[2,j,i],max_P5[2,j])
                    min_P5[2,j]=min(predict_P5_lower[2,j,i],min_P5[2,j])
                end
            end
        end
        for i in 1:(nptss)
            if (llP5_upper[i].-maximum(llP5)) >= TH
                for j in 1:length(t_smooth)
                    max_P5[1,j]=max(predict_P5_upper[1,j,i],max_P5[1,j])
                    min_P5[1,j]=min(predict_P5_upper[1,j,i],min_P5[1,j]) 
                    max_P5[2,j]=max(predict_P5_upper[2,j,i],max_P5[2,j])
                    min_P5[2,j]=min(predict_P5_upper[2,j,i],min_P5[2,j])
                end
            end
        end
        # combine the confidence sets for realisations
        max_realisations_P5=zeros(2,length(t_smooth))
        min_realisations_P5=1000*ones(2,length(t_smooth))
        for i in 1:(nptss)
            if (llP5_lower[i].-maximum(llP5)) >= TH_realisations
                for j in 1:length(t_smooth)
                    max_realisations_P5[1,j]=max(predict_P5_realisations_lower_uq[1,j,i],max_realisations_P5[1,j])
                    min_realisations_P5[1,j]=min(predict_P5_realisations_lower_lq[1,j,i],min_realisations_P5[1,j])
                    max_realisations_P5[2,j]=max(predict_P5_realisations_lower_uq[2,j,i],max_realisations_P5[2,j])
                    min_realisations_P5[2,j]=min(predict_P5_realisations_lower_lq[2,j,i],min_realisations_P5[2,j])
                end
            end
        end
        for i in 1:(nptss)
            if (llP4_upper[i].-maximum(llP5)) >= TH_realisations
                for j in 1:length(t_smooth)
                    max_realisations_P5[1,j]=max(predict_P5_realisations_upper_uq[1,j,i],max_realisations_P5[1,j])
                    min_realisations_P5[1,j]=min(predict_P5_realisations_upper_lq[1,j,i],min_realisations_P5[1,j]) 
                    max_realisations_P5[2,j]=max(predict_P5_realisations_upper_uq[2,j,i],max_realisations_P5[2,j])
                    min_realisations_P5[2,j]=min(predict_P5_realisations_upper_lq[2,j,i],min_realisations_P5[2,j])
                end
            end
        end
       ...
       # Plot Profile likelihoods 
        ...
        profile4=vline!([K2],lw=3,linecolor=:rosybrown,linestyle=:dash)
         ...
         interp_points_P5range =  LinRange(P5min,P5max,interp_nptss)
         interp_P5 = LinearInterpolation(P5range,nllP5)
         interp_nllP5 = interp_P5(interp_points_P5range)
         
         profile5=plot(interp_points_P5range,interp_nllP5,xlim=(P5min,P5max),ylim=(-4,0.1),yticks=[-3,-2,-1,0],xlab=L"\sigma_{\mathrm{N}}",ylab=L"\hat{\ell}_{p}",legend=false,lw=5,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=:deepskyblue3)
         profile5=hline!([-1.92],lw=2,linecolor=:black,linestyle=:dot)
         profile5=vline!([P5mle],lw=3,linecolor=:red)
        ...
        display(profile5)
        savefig(profile5,filepath_save[1] * "Fig_profile5"    * ".pdf")
        ...
        # Confidence sets for model solutions
           ...
           savefig(confmodel4,filepath_save[1] * "Fig_confmodel4"   * ".pdf")
            ...
            # P5
            confmodel5=plot(t_smooth,ymle_smooth[1,:],lw=3,linecolor=colour1) # solid - c1
            confmodel5=plot!(t_smooth,ymle_smooth[2,:],xlab=L"t",ylab=L"c_{1}(t), c_{2}(t)",lw=3,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2) # solid - c2
            confmodel5=scatter!(t,data[1,:],xlims=(0,t_max+1),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour1,msw=0) # scatter c1
            confmodel5=scatter!(t,data[2,:],xlims=(0,t_max+1),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour2,msw=0) # scatter c2
            confmodel5=plot!(t_smooth,ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_P5[1,:],max_P5[1,:].-ymle_smooth[1,:]),fillalpha=.2)
            confmodel5=plot!(t_smooth,ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_P5[2,:],max_P5[2,:].-ymle_smooth[2,:]),fillalpha=.2)
            confmodel5=hline!([0],lw=2,linecolor=:black,linestyle=:dash)

            display(confmodel5)
            savefig(confmodel5,filepath_save[1] * "Fig_confmodel5"   * ".pdf")
           ...
           for j in 1:length(t_smooth)
               max_overall[1,j]=max(max_P1[1,j],max_P2[1,j],max_P3[1,j],max_P4[1,j],max_P5[1,j])
               max_overall[2,j]=max(max_P1[2,j],max_P2[2,j],max_P3[2,j],max_P4[2,j],max_P5[2,j])
               min_overall[1,j]=min(min_P1[1,j],min_P2[1,j],min_P3[1,j],min_P4[1,j],min_P5[1,j])
               min_overall[2,j]=min(min_P1[2,j],min_P2[2,j],min_P3[2,j],min_P4[2,j],min_P5[2,j])
           end
...
#########################################################################################
        ...
        # P5
        confmodeldiff5=plot(t_smooth,0 .* ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_P5[1,:],max_P5[1,:].-ymle_smooth[1,:]),ylims=(ydiffmin,ydiffmax),legend=false,fillalpha=.2,lw=0,linecolor=colour1)
        confmodeldiff5=plot!(t_smooth,0 .* ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_P5[2,:],max_P5[2,:].-ymle_smooth[2,:]),fillalpha=.2,xlab=L"t",ylab=L"C_{y,0.95}^{\sigma_{N}} - y(\hat{\theta})",lw=0,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2)
        confmodeldiff5=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash)
        
        display(confmodeldiff5)
        savefig(confmodeldiff5,filepath_save[1] * "Fig_confmodeldiff5"   * ".pdf")
...
#########################################################################################
   ...
   savefig(confreal3,filepath_save[1] * "Fig_confreal3"   * ".pdf")
    ...
    # P5
    confreal5=plot(t_smooth,ymle_smooth[1,:],lw=3,linecolor=colour1) # solid - c1
    confreal5=plot!(t_smooth,ymle_smooth[2,:],xlab=L"t",ylab=L"c_{1}(t), c_{2}(t)",lw=3,titlefont=fnt, guidefont=fnt, tickfont=fnt, legendfont=fnt,linecolor=colour2) # solid - c2
    confreal5=scatter!(t,data[1,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour1,msw=0) # scatter c1
    confreal5=scatter!(t,data[2,:],xlims=(xmin,xmax),ylims=(ymin,ymax),legend=false,markersize = 8,markercolor=colour2,msw=0) # scatter c2
    confreal5=plot!(t_smooth,ymle_smooth[1,:],w=0,c=colour1,ribbon=(ymle_smooth[1,:].-min_realisations_P5[1,:],max_realisations_P5[1,:].-ymle_smooth[1,:]),fillalpha=.2)
    confreal5=plot!(t_smooth,ymle_smooth[2,:],w=0,c=colour2,ribbon=(ymle_smooth[2,:].-min_realisations_P5[2,:],max_realisations_P5[2,:].-ymle_smooth[2,:]),fillalpha=.2)
    confreal5=hline!([0],lw=2,linecolor=:black,linestyle=:dash)
    

    display(confreal5)
    savefig(confreal5,filepath_save[1] * "Fig_confreal5"   * ".pdf")
   ...
   for j in 1:length(t_smooth)
       max_realisations_overall[1,j]=max(max_realisations_P1[1,j],max_realisations_P2[1,j],max_realisations_P3[1,j],max_realisations_P4[1,j],max_realisations_P5[1,j])
       max_realisations_overall[2,j]=max(max_realisations_P1[2,j],max_realisations_P2[2,j],max_realisations_P3[2,j],max_realisations_P4[2,j],max_realisations_P5[2,j])
       min_realisations_overall[1,j]=min(min_realisations_P1[1,j],min_realisations_P2[1,j],min_realisations_P3[1,j],min_realisations_P4[1,j],min_realisations_P5[1,j])
       min_realisations_overall[2,j]=min(min_realisations_P1[2,j],min_realisations_P2[2,j],min_realisations_P3[2,j],min_realisations_P4[2,j],min_realisations_P5[2,j])
   end
