...
# Fig2_ODELinear_LogNormalNoise_NormalFit
...
isdir(pwd() * "\\Fig2_ODELinear_LogNormalNoise_NormalFit\\") || mkdir(pwd() * "\\Fig2_ODELinear_LogNormalNoise_NormalFit") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\Fig2_ODELinear_LogNormalNoise_NormalFit\\"] # location to save figures
filepath_load = [pwd() * "\\Fig4_ODELinear_LogNormalNoise_LogNormalFit\\"] # location to load data
...
#######################################################################################
## Load synthetic data generated using the Lognormal noise model
...
#######################################################################################
### Plot data - true solution and scatter
...
### Parameter identifiability analysis - MLE, profiles
    ...
    #######################################################################################
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr1range_upper[2,i]) for mi in modelmean]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr1range_lower[2,i]) for mi in modelmean]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr2range_upper[2,i]) for mi in modelmean]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,nr2range_lower[2,i]) for mi in modelmean]; # normal distribution about mean
        ...
        max_r2_Sd= zeros(2,length(t_smooth))
        min_r2_Sd= zeros(2,length(t_smooth))
        for i in 1:(nptss)
            if (llr2_lower[i].-maximum(llr2)) >= TH
                for j in 1:length(t_smooth)
                    if predict_r2_lower[1,j,i] > max_r2[1,j]
                        max_r2[1,j]=max(predict_r2_lower[1,j,i],max_r2[1,j])
                        max_r2_Sd[1,j]=nr2range_lower[1,i]
                    end
                    if predict_r2_lower[1,j,i] < min_r2[1,j]
                        min_r2[1,j]=min(predict_r2_lower[1,j,i],min_r2[1,j])
                        min_r2_Sd[1,j]=nr2range_lower[1,i]
                    end
                    if predict_r2_lower[2,j,i] > max_r2[2,j]
                        max_r2[2,j]=max(predict_r2_lower[2,j,i],max_r2[2,j])
                        max_r2_Sd[2,j]=nr2range_lower[2,i]
                    end
                    if predict_r2_lower[2,j,i] < min_r2[2,j]
                        min_r2[2,j]=min(predict_r2_lower[2,j,i],min_r2[2,j])
                        min_r2_Sd[2,j]=nr2range_lower[2,i]
                    end
                end
            end
        end
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,Sdrange_upper[i]) for mi in modelmean]; # normal distribution about mean
        ...
        for i in 1:nptss
            ...
            loop_data_dists=[Normal(mi,Sdrange_lower[i]) for mi in modelmean]; # normal distribution about mean
...
#########################################################################################
        ...
        confmodel1=scatter!(t,data[1,:],xlims=(0,5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour1,markerstrokewidth=0) # scatter c1
        confmodel1=scatter!(t,data[2,:],xlims=(0,5.5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour2,markerstrokewidth=0) # scatter c2
        ...
        confmodel2=scatter!(t,data[1,:],xlims=(0,5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour1,markerstrokewidth=0) # scatter c1
        confmodel2=scatter!(t,data[2,:],xlims=(0,5.5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour2,markerstrokewidth=0) # scatter c2
        ...
        confmodelu=scatter!(t,data[1,:],xlims=(0,5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour1,markerstrokewidth=0) # scatter c1
        confmodelu=scatter!(t,data[2,:],xlims=(0,5.5),ylims=(0,125),legend=false,markersize = 8,markercolor=colour2,markerstrokewidth=0) # scatter c2
...
#########################################################################################
        ...
        confmodeldiff1=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash,xlims=(xmin,xmax))
        ...
        confmodeldiff2=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash,xlims=(xmin,xmax))
        ...
        confmodeldiffu=plot!(t_smooth,0 .*ymle_smooth[2,:],lw=1,c=:black,linestyle=:dash,xlims=(xmin,xmax))
