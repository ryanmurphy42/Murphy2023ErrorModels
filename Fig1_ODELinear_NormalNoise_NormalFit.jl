...
# Fig1_ODELinear_NormalNoise_NormalFit
...
isdir(pwd() * "\\Fig1_ODELinear_NormalNoise_NormalFit\\") || mkdir(pwd() * "\\Fig1_ODELinear_NormalNoise_NormalFit") # make folder to save figures if doesnt already exist
filepath_save = [pwd() * "\\Fig1_ODELinear_NormalNoise_NormalFit\\"] # location to save figures "\\Fig2\\"] # location to save figures
...
### Parameter identifiability analysis - MLE, profiles
    ...
    #######################################################################################
        ...
        p_residuals=scatter(t,data_residuals[1,:],xlims=(xmin,xmax),ylims=(-15,15),legend=false,markersize = 15,markercolor=colour1,markerstrokewidth=0) # scatter c1
